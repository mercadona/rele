...
class TestPublisher:
    ...
    def test_publishes_data_with_custom_encoder(
        self, publisher, custom_encoder
    ):
        publisher._encoder = custom_encoder
        publisher.publish(
            topic="order-cancelled", data=decimal.Decimal("1.20")
        )

        publisher._client.publish.assert_called_with(
            ANY, b"1.2", published_at=ANY
        )
    ...
    def test_publishes_data_with_client_timeout_when_blocking(
        ...
        publisher.publish(
            topic="order-cancelled", data={"foo": "bar"}, blocking=True
        )
...
class TestSubscriber:
    ...
    def test_creates_subscription_with_default_ack_deadline_when_none_provided(
        ...
        expected_subscription = (
            f"projects/{project_id}/subscriptions/" f"test-topic"
        )
        expected_topic = (
            f"projects/{project_id}/topics/" f"{project_id}-test-topic"
        )

        subscriber.create_subscription(
            "test-topic", f"{project_id}-test-topic"
        )
    ...
    def test_logs_error_when_subscription_topic_does_not_exist(
        ...
        log = caplog.records[0]
        assert (
            log.message == "Cannot subscribe to a topic that does not exist."
        )
